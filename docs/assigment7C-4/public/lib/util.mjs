 ...
 * */
function createListFromMap(et, displayProp, displayProp2, displayProp3) {
  const listEl = document.createElement("ul");
  if(!displayProp2 ) {
    fillListFromMap(listEl, et, displayProp, null, null);
  }else if (!displayProp3 ){
    fillListFromMap(listEl, et, displayProp, displayProp2, null );
  }else {
    fillListFromMap(listEl, et,displayProp, displayProp2, displayProp3 )
  }
  return listEl;
}
/**
 ...
 * @param {string} displayProp  The object property to be displayed in the list
 */
function fillListFromMap(listEl, et, displayProp, displayProp2, displayProp3 ) {
  ...
  for (const key of keys) {
    ...
    if(!displayProp2) {
      ...
      listItemEl.textContent = `Amount: ` + et[key][displayProp] + " Name: " +et[key][displayProp2] + " Price: " +et[key][displayProp3];
    }
 ...
 */
function fillChoiceSet( listEl, selection, keyProp, displayProp, displayProp2, displayProp3) {
...
function addItemToListOfSelectedItems( targetObjt, idRefTargetName , classValue) {
  ...
  let targetObjText
  if(targetObjt.productName){
    listItemEl.innerText = `${targetObjt[idRefTargetName]}: ${targetObjt.amount} x ${targetObjt.productName} : ${targetObjt.total} $`.substring(0, 25);
    // convert target object into text
    targetObjText = JSON.stringify({id: targetObjt[idRefTargetName], name: targetObjt.productName, amount: targetObjt.amount, price: targetObjt.total});
    // embed target object in list item (li element)
  }else {
    listItemEl.innerText = `${targetObjt[idRefTargetName]}: ${targetObjt.amount} x ${targetObjt.name}: ${targetObjt.price} $`.substring(0, 25);
    // convert target object into text
    targetObjText = JSON.stringify({id: targetObjt[idRefTargetName], name: targetObjt.name, amount: targetObjt.amount, price: targetObjt.price});
    // embed target object in list item (li element)
  }
...
export { createChoiceWidget, isIntegerOrIntegerString,createModalFromChange,fillSelectWithOptions, createListFromMap, createMultiSelectionWidget,
