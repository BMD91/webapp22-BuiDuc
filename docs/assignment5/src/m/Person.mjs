    ...
    if ((constraintViolation instanceof NoConstraintViolation) && id) {
      if (!Person.instances[String(id)]) {
        constraintViolation = new ReferentialIntegrityConstraintViolation(
            `${id}There is no person record with this person ID!`);
      }
    }
    return constraintViolation;
  }
...
Person.destroy = function (personId) {
  ...
  for (const mid of Object.keys( person.playedMovies)) {
    let movie = person.playedMovies[mid];
    if (movie.actors[personId]) delete movie.persons[personId];
  }
  for (const mid2 of Object.keys( person.directedMovies)) {
    let movie2 = person.playedMovies[mid2];
    if (movie2.actors[personId]) delete movie2.persons[personId];
  }
